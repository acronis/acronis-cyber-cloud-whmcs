 ...
 * @link        https://github.com/swagger-api/swagger-codegen
 */
class IdpPut extends BaseModel 
{
    ...
    /**
      ...
      * @var string
      */
    protected static $swaggerModelName = 'IdentityProviders\IdpPut';
    ...
    {
        if (!is_null($owner_tenant_id) && (strlen($owner_tenant_id) > 36)) {
            throw new \InvalidArgumentException('invalid length for $owner_tenant_id when calling IdpPut., must be smaller than or equal to 36.');
        }
        if (!is_null($owner_tenant_id) && (strlen($owner_tenant_id) < 36)) {
            throw new \InvalidArgumentException('invalid length for $owner_tenant_id when calling IdpPut., must be bigger than or equal to 36.');
        }
        if (!is_null($owner_tenant_id) && (!preg_match("/[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}/", $owner_tenant_id))) {
            throw new \InvalidArgumentException("invalid value for $owner_tenant_id when calling IdpPut., must conform to the pattern /[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}/.");
        }
    ...
    public function setAddress($address)
    {
        ...
        if (!is_null($address) && (!preg_match("/^[a-z0-9\\+]+:\/\/[^\\s\/$.?#].[^\\s]*/", $address))) {
            throw new \InvalidArgumentException("invalid value for $address when calling IdpPut., must conform to the pattern /^[a-z0-9\\+]+:\/\/[^\\s\/$.?#].[^\\s]*/.");
        }
